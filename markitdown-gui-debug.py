...
class MarkItDownConverter:
    ...
    def convert(self, file_path):
        ...
        print(f"[DEBUG] 支持的格式: {list(self.converters.keys())}")
...
class MarkItDownGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("MarkItDown 转换器 (调试版)")
        self.root.geometry("700x500")
        
        print("[DEBUG] 初始化转换器...")
        self.converter = MarkItDownConverter()
        print(f"[DEBUG] 转换器支持格式: {list(self.converter.converters.keys())}")
    ...
    def create_widgets(self):
        ...
        supported_formats = f"支持格式: {', '.join(sorted(self.converter.converters.keys()))}"
    ...
    def convert_files(self):
        ...
        print(f"[DEBUG] 转换完成: 成功 {success_count} 个, 失败 {fail_count} 个")
