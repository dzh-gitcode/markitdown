...
class MarkItDownGUI:
    ...
    def create_widgets(self):
        ...
        default_output = os.path.join(os.path.expanduser("~"), "Documents", "MarkItDown输出")
    ...
    def convert_files(self):
        ...
        try:
            ...
            messagebox.showerror("权限错误", f"无法写入输出目录: {output_dir}\n\n请选择其他目录或以管理员身份运行程序")
            return
        except Exception as e:
            messagebox.showerror("错误", f"无法写入输出目录: {e}")
            return
        ...
        for i, file_path in enumerate(self.selected_files):
            try:
                ...
                fail_count += 1
                failed_files.append(f"{os.path.basename(file_path)}: 权限不足，无法写入文件")
            except Exception as e:
                fail_count += 1
